...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  const staff = await requireStaff(["STAFF", "SUPER_ADMIN", "MARKETING"]);
  if (!staff) return NextResponse.json({ error: "ไม่มีสิทธิ์เข้าถึง" }, { status: 403 });

  // STAFF is tied to one branch already; HQ-level roles (no branchId) must say
  // which branch handed the reward out via the request body.
