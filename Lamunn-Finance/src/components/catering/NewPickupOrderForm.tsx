...
export default function NewPickupOrderForm({ branches }: { branches: BranchOption[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setError(null);
    const res = await fetch("/api/catering/pickup-orders", {
      method: "POST",
    ...
    router.push(`/catering/pickup-orders/${body.order.id}`);
    router.refresh();
  }
    ...
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-5">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="sm:col-span-3">
          <p className="text-xs text-gray-400">ติ๊กรายการที่ลูกค้าสั่งได้อีกทีในหน้ารายละเอียดหลังบันทึก — ไม่ต้องพิมพ์ตรงนี้แล้ว</p>
        </div>
        <div className="sm:col-span-3">
