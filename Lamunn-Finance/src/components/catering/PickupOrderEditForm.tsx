...
export default function PickupOrderEditForm({ order, branches, hasItems = false }: { order: OrderData; branches: BranchOption[]; hasItems?: boolean }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    setSaved(false);
    const res = await fetch(`/api/catering/pickup-orders/${order.id}`, {
      method: "PATCH",
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div>
          <label className="mb-1 flex items-center gap-1.5 text-xs font-medium text-gray-500">
            ยอดเงิน (บาท)
            {hasItems && <span className="rounded bg-brand-50 px-1.5 py-0.5 text-[10px] font-normal text-brand-600">คำนวณจากรายการด้านล่าง</span>}
          </label>
          <input
            required
            type="number"
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-500">เวลานัดรับ</label>
