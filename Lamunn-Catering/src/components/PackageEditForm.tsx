...
export default function PackageEditForm({ pkg }: { pkg: PackageData }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/packages/${pkg.id}`, {
      ...
      body: JSON.stringify({ name, description, price: Number(price), sortOrder, isActive }),
    });
    setSaving(false);
    if (!res.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-5">
      <h2 className="mb-4 text-sm font-semibold text-gray-700">แก้ไขแพคเกจ</h2>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-500">ชื่อแพคเกจ</label>
          <input
            required
            value={name}
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-500">ราคา (บาท)</label>
          <input
            required
            type="number"
            step="0.01"
