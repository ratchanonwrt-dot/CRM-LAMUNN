...
function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    setLoading(true);
    const res = await signIn("catering-credentials", { email, password, redirect: false, callbackUrl });
    setLoading(false);
    if (res?.error) {
      ...
      <div className="mx-auto flex w-full max-w-sm flex-col gap-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            <h1 className="text-lg font-bold text-brand-700">Lamunn Catering</h1>
            <p className="text-sm text-gray-500">ระบบบันทึกการจองจัดเลี้ยง</p>
          </div>
        </div>
