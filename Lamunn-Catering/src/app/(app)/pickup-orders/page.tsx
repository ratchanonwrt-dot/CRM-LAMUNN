  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-2">
        ...
        <Link href="/pickup-orders/new" className="rounded-xl bg-brand-600 px-5 py-2.5 text-sm font-medium text-white shadow-md shadow-brand-600/20 hover:bg-brand-700">
          + บันทึกออเดอร์ใหม่
        </Link>
      </div>
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.id} className="border-t border-gray-100">
                <td className="px-4 py-2 text-gray-700">{formatThaiDate(o.pickupDate)}</td>
                ...
                <td className="px-4 py-2 text-gray-500">{o.itemsDescription}</td>
                <td className="px-4 py-2 text-gray-500">{o.branch?.name ?? "-"}</td>
                <td className="px-4 py-2 text-gray-700">{formatBaht(o.amount)}</td>
                <td className="px-4 py-2 text-gray-500">{paymentStatusLabel[o.paymentStatus]}</td>
                <td className="px-4 py-2 text-gray-500">{pickupStatusLabel[o.status]}</td>
                <td className="px-4 py-2">
                  <Link href={`/pickup-orders/${o.id}`} className="text-brand-600 hover:underline">
                    ดู/แก้ไข
                  </Link>
                </td>
              </tr>
